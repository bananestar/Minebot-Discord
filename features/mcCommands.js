...
function parseMcCommand(msg) {
	const match = msg.match(/^<(\w+)>\s*!bot\s*(.*)/i) || msg.match(/^(\w+):\s*!bot\s*(.*)/i);
...
const COMMANDS = {
	...
	tpa: {
		description: 'Envoie une demande de TPA vers un joueur (ex: !bot tpa Pseudo)',
		run({ bot, args, Logger }) {
			if (args.length !== 1) {
	...
	goto: {
		description: 'Deplace le bot vers des coordonnees XYZ (ex: !bot goto 100 64 -200)',
		async run({ bot, args, Logger }) {
			if (args.length !== 3) {
